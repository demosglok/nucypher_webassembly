...
export default function languagePluginLoader(baseURL) {
  return new Promise((resolve, reject) => {
  // This is filled in by the Makefile to be either a local file or the
  // deployed location. TODO: This should be done in a less hacky
  // way.
  //var baseURL = self.languagePluginUrl || 'https://iodide.io/pyodide-demo/';
  if(!baseURL) baseURL = 'https://iodide.io/pyodide-demo/';
  ...
  let _loadPackage = (names, messageCallback) => {
    console.log('_loadPackage', self, self.pyodide);
    ...
    while (queue.length) {
      ...
      if (package_name in loadedPackages) {
      ...
      } else if (package_name in toLoad) {
        if (package_uri != toLoad[package_name]) {
          console.error(`URI mismatch, attempting to load package ` +
                        `${package_name} from ${package_uri} while it is already ` +
                        `being loaded from ${toLoad[package_name]}!`);
          return;
        }
      } else {
        ...
        if (packages.hasOwnProperty(package_name)) {
          ...
          console.error(`Unknown package '${package_name}'`);
        }
      }
    }
    ...
    self.pyodide._module.locateFile = (path) => {
      ...
      let package_name = path.replace(/\.data$/, "");
      if (package_name in toLoad) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      self.pyodide._module.monitorRunDependencies = () => {
        ...
        if (packageCounter === 0) {
          for (let package_name in toLoad) {
            self.pyodide.loadedPackages[package_name] = toLoad[package_name];
          }
      ...
      for (let package_name in toLoad) {
        ...
        let package_uri = toLoad[package_name];
        if (package_uri == 'default channel') {
          scriptSrc = `${baseURL}${package_name}.js`;
        } else {
        ...
        loadScript(scriptSrc, () => {}, () => {
          ...
          let index = toLoad.indexOf(package_name);
          if (index !== -1) {
  ...
  let Module = {};
  console.log('loading Pyodide', self)
  self.Module = Module;
  ...
  loadScript(data_script_src, () => {
    ...
    loadScript(scriptSrc, () => {
      ...
      console.log('after loading pyodide', self);
